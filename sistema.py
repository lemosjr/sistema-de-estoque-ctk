...
class ItemManager:
    ...
    def create_input_frame(self):
        ...
        label_type = ctk.CTkLabel(frame_data_items, text="Alcoolico:")
    ...
    def create_list_frame(self):
        ...
        self.tree = ttk.Treeview(frame_registered_items, columns=('Id', 'Nome', 'Alcoolico', 'Marca', 'Quantidade'), show="headings")
        self.tree.heading('Id', text='Id')
        self.tree.heading('Nome', text='Nome')
        self.tree.heading('Alcoolico', text='Alcoolico')
        self.tree.heading('Marca', text='Marca')
        self.tree.heading('Quantidade', text='Quantidade')

        self.tree.column('Id', width=125, anchor='center')
        self.tree.column('Nome', width=125)
        self.tree.column('Alcoolico', width=125)
    ...
    def register_item(self):
        ...
        if name and item_type and quality and amount:
            ...
            if self.selected_item_id_to_edit is not None:
                for i in range(len(self.itens)):
                    if self.itens[i]['Id'] == self.selected_item_id_to_edit:
                        self.itens[i].update({'Nome': name, 'Alcoolico': item_type, 'Marca': quality, 'Quantidade': amount})
                        break
            ...
            # Caso contrário, cria um novo item.
                ...
                self.item_id += 1
                novo_item = {
                    "Id": self.item_id, "Nome": name, "Alcoolico": item_type,
